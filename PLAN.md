...
1. **NDJSON streaming under s&box `Http`** — project-critical, no fallback for Board API; spiked in M3 (TV feed, no auth). **RESULT (2026-07-13): incremental streaming does NOT work** — `await Http.RequestAsync` on the endless TV feed never returns (buffers the whole body). Board API play (M4) is blocked until s&box exposes a headers-first / streaming HTTP read; mitigation = Facepunch whitelist/feature request (no lichess polling alternative exists). `gambit_lichess_tv` reproduces it.
