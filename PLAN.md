...
- **M3 — Lichess auth** *(code-complete 2026-07-13, gate pending user test)*: **First, carry-in from M2:** fix the broken game-over lichess import (the "Get lichess link" button copies nothing — see the M2 line above). It is the first live lichess request, so debugging it here also validates the HttpAllowList before the auth/streaming work depends on it. Then: Spike 1 FIRST: ndjson streaming via `/api/tv/feed` (D4 — project-critical). Spike 2: browser-open + loopback feasibility (D3). Then `LichessAuth`, token storage, SplashScreen, `GET /api/account` name/rating in name tag. Gate: sign in, token persists across restart, 401 → re-prompt.
  **Built (M3):** all lichess REST now goes through `Api/LichessApi.cs` — one seam owning the CLAUDE.md rate discipline (single-flight gate + 60s-429 backoff) and the **4-arg `Http.RequestAsync(url,method,content,headers)`** shape (the fix for the dead M2 import: the old 3-arg call didn't land the form body). `Api/LichessModels.cs` DTOs, `Api/LichessAuth.cs` (token-paste sign-in → validate via `GET /api/account` → store token+username+rating in `PlayerData`; sign-out revokes via `DELETE /api/token`; `OnUnauthorized` clears+re-prompts; `ValidateStoredToken` re-checks on load), `UI/Screens/SplashScreen.razor` (screen-space modal self-attached like GameHud; copy-scoped-token-link + paste flow, or anonymous display-name; cursor freed via `UseLookControls=false`+`UseInputControls=false`, restored on close), name tag shows lichess name+rating (`[Sync] GambitRating`). **Spike verdicts:** D3 spike 2 is settled by construction — no whitelisted browser-open API and `HttpListener`/sockets are outside the whitelist (referencing them is SB1000), so **token-paste is the shipped flow and PKCE stays deferred** until a whitelisted loopback exists. D4 spike 1 must be **user-run in-editor**: `gambit_lichess_tv` streams the TV feed and logs per-line arrival gaps — trickling lines ⇒ incremental streaming ⇒ Board API viable; if "response headers received" never logs, `RequestAsync` buffers and the D4 mitigation (Facepunch whitelist request) is needed. Console commands (dev): `gambit_lichess_tv`, `gambit_lichess_import_test` (self-tests the POST with a known-good PGN, isolating HTTP from our PGN builder), `gambit_signin [token]`, `gambit_signout`, `gambit_whoami`. Token hygiene: only ever in the Authorization header, `LichessApi.Redact` for logs, never `[Sync]`/RPC'd. **M3 gate (user, in-editor):** `gambit_lichess_tv` → confirm incremental streaming (or report the blocked symbol); `gambit_lichess_import_test` PASS (also closes M2's import gate) → then a real game-over "Get lichess link" works; open splash (auto on first run, or `gambit_signin`), paste a `lip_` token, sign in, name tag shows the lichess name+rating; restart → still signed in; revoke the token on lichess then reload → 401 clears it and re-pops the splash.
