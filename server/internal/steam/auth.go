// Package steam validates s&box (Facepunch) auth tokens server-side so gamchess
// can trust a client-supplied SteamID64 instead of taking it on faith.
//
...
//
// Ported from rotaliate/internal/steam/auth.go — this is the trust boundary for
// the whole service, so it is a deliberate copy of code already proven in
// production rather than a fresh implementation.
package steam
...
// is the decimal SteamID64 string the client sent — an unverified *claim*, used
// solely for the equality check below and never as an identity. Any transport
// error, non-200 response, bad status, or SteamId mismatch returns (false, err|nil)
// — callers must treat a non-true result as "deny", never as "allow on error".
func ValidateToken(ctx context.Context, steamID64, token string) (bool, error) {
	...
	// authorise this steamID. This single line is what stops a real token for
	// account Y from authorising as account X.
