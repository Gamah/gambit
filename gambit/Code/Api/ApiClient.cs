...
public static class ApiClient
{
	...
	/// at a chosen instance via <see cref="Gambit.World.LobbyNetworkManager.TargetUrl"/>.</summary>
	...
	public static async Task<RecoverBySteamResponse> RecoverBySteamId()
	{
		...
		if ( !resp.IsSuccessStatusCode )
		{
			Log.Warning( $"[Gambit] SteamID recovery failed: HTTP {(int)resp.StatusCode}" );
			return null;
		}
	...
	/// name tracks the selected backend (prod = "gambit", test = "gambit-test")
	...
	{
		var service = BaseUrl == TestUrl ? "gambit-test" : "gambit";
		try { return await Sandbox.Services.Auth.GetToken( service ); }
		catch ( Exception e )
		{
			Log.Warning( $"[Gambit] auth token unavailable: {e.Message}" );
			return null;
		}
	}
	...
	public static async Task LinkSteamId( string guid )
	{
		...
		if ( !resp.IsSuccessStatusCode )
		{
			Log.Warning( $"[Gambit] SteamID link failed: HTTP {(int)resp.StatusCode}" );
			return;
		}
