...
public sealed class GameController : Component
{
	...
	public async Task StartGame( GameMode mode, string seed = null )
	{
		...
		if ( string.IsNullOrEmpty( SessionId ) )
			Log.Warning( "[Gambit] No session_id from server — moves won't be recorded." );
	...
	async Task SendMoveChained( Task prev, string session, int encoded )
	{
		...
		{
			_sessionLost = true;
			Log.Warning( $"[Gambit] Move rejected (HTTP {e.StatusCode}) — server session out of sync; this run won't be recorded." );
		}
		catch ( Exception e )
		{
			_sessionLost = true;
			Log.Warning( $"[Gambit] Move send failed ({e.Message}) — server session out of sync; this run won't be recorded." );
		}
	}
