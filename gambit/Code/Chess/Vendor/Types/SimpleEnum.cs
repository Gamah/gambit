...
public abstract class SimpleEnum<T> where T : SimpleEnum<T>
{
    ...
    protected SimpleEnum(string name, int value)
    {
        ...
        // GAMBIT VENDOR PATCH (s&box whitelist): GetType()==typeof(T) guarded
        // against subclasses of the concrete enum; none exist, and a type
        // pattern match does the same without reflection.
        if (!ValueMap.ContainsKey(value) && this is T instance)
        {
            ValueMap[value] = instance;
