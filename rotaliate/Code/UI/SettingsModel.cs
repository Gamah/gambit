...
public static class SettingsModel
{
	...
	{
		("LIVE", Rotaliate.Api.ApiClient.ProdUrl),
		("TEST", Rotaliate.Api.ApiClient.TestUrl),
	};
	...
	public static string TargetServerLabel()
	{
		foreach ( var (label, url) in Instances )
			if ( Rotaliate.Api.ApiClient.BaseUrl == url ) return label;
		return "CUSTOM";
	}
	...
	static SettingRow TargetUrlRow( Scene scene )
	{
		...
		foreach ( var (label, url) in Instances )
		{
			...
			{
				Label = label,
				Selected = Rotaliate.Api.ApiClient.BaseUrl == target,
