...
/// <summary>
/// A wall screen the player can play Rotaliate on. One per screen instance —
...
public sealed class ArcadeStation : Component
{
	...
	// Rotaliate GUID is a secret backend credential and is NEVER replicated to lobby
	...
	async void ValidateIdentity()
	{
		...
		try
		{
			if ( !await ApiClient.PlayerExists( guid ) && Active == this )
			{
				...
				Log.Warning( $"[Rotaliate] Player {Redact( guid )} unknown to server — re-enrolling" );
		...
		{
			Log.Warning( $"[Rotaliate] Identity check failed: {e.Message}" );
		}
	}
	...
	void UpdateDemoText()
	{
		if ( DemoActive && !_demoTextGo.IsValid() )
		{
			...
			_demoTextGo.AddComponent<Rotaliate.UI.DemoInfoPanel>().Station = this;
		}
