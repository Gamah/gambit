...
public sealed class ArcadeRing : Component, Component.ExecuteInEditor
{
	...
	IReadOnlyList<GameObject> BuildInternal()
	{
		...
		for ( int i = 0; i < count; i++ )
		{
			...
			screen.AddComponent<Rotaliate.UI.ArcadeScreenPanel>();
		...
		Rotation? localRot = null, Color? tint = null )
	{
		...
		if ( model == null )
		{
			Log.Warning( "[Rotaliate] models/dev/box.vmdl failed to load — arcade cabinets have colliders but no visuals" );
			return;
		}
	...
	void AddButtonGlyph( GameObject parent, Vector3 localPos, string glyph )
	{
		...
		go.AddComponent<Rotaliate.UI.ButtonGlyphPanel>().Glyph = glyph;
	}
	...
	void AddMarqueeNumber( GameObject cabinet, int index )
	{
		...
		go.AddComponent<WorldPanel>();
		go.AddComponent<Rotaliate.UI.MarqueeNumberPanel>().Number = index.ToString();
	}
