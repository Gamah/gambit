...
public sealed class RoomLightOrbit : Component, Component.ExecuteInEditor
{
	...
	void BuildSun()
	{
		...
		if ( model == null )
		{
			Log.Warning( "[Rotaliate] models/dev/sphere.vmdl failed to load — no sun visual" );
			return;
		}
	...
	void ApplyBrightness()
	{
		...
		if ( _playMode && Rotaliate.UI.SettingsModel.SettingsVersion != _appliedVersion )
		{
			var data = Rotaliate.Game.PlayerData.Load();
			_userMul = Rotaliate.Game.PlayerData.ClampLightScale( data?.WorldLightBrightness ?? 1f );
			_appliedVersion = Rotaliate.UI.SettingsModel.SettingsVersion;
		}
