...
public sealed class MultiplayerController : Component
{
	...
	public async Task Connect( bool is2p )
	{
		...
		if ( string.IsNullOrEmpty( player.PlayerTag ) )
		{
			...
			{
				Log.Warning( $"[Rotaliate/MP] player_tag fetch failed: {e.Message}" );
			}
		}
		...
		{
			Log.Warning( $"[Rotaliate/MP] ws ticket mint failed: {e.Message}" );
		...
		{
			Log.Warning( $"[Rotaliate/MP] connect failed: {e.Message}" );
			Error = "Could not connect to server.";
			State = MpState.Idle;
			OnStateChanged?.Invoke();
		}
	...
	void OnWsMessage( string raw )
	{
		...
		{
			Log.Warning( $"[Rotaliate/MP] parse error: {e.Message}" );
		}
	}
